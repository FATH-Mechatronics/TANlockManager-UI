<template>
    <div>
        ...
        <CabinetEditDialog
                ...
                :rows="rows"
                :locks="locks"
                @closed="closedAddDialog"
        />
        <RowEditDialog :cage="cage" :row="row" :dialog="editDialog" @closed="closedEditDialog"/>
...
<script lang="ts">
    ...
    class RowComponent extends Vue {
        ...
        closedEditDialog(newRow: Row){
            this.editDialog = false;
            if(newRow != null){
                let oldId = this.rows.findIndex(r=>r.id === newRow.id);
                if(oldId >= 0){
                    this.rows.splice(oldId,1,newRow);
                }else{
        ...
        closedAddDialog(newCabinet: Cabinet | null) {
            this.addDialog = false;
            if (newCabinet !== null) {
                const oldIndex = this.cabinets.findIndex(c => c.id == newCabinet.id);
                if (oldIndex >= 0) {
                    this.cabinets.splice(oldIndex, 1, newCabinet)
                } else {
        ...
        get gradientStyle() {
            if(this.row.gradient !== "transparent") {
